...
X <- as.matrix(dat_train[,-c(1,8)])
Y <- dat_train$Survived
...
pred.rf1 <- predict(rf1, dat_test)
table(pred.rf1, dat_test$Survived)
yhat.rf1 <- predict(rf1, dat_test)
yhat.rf1

# let's look at the ROC curve and AUC
library(pROC)
rf1.roc <- roc(dat_test$Survived, yhat.rf1, direction="<")
rf1.roc
plot(rf1.roc, lwd=3)

# predictions new data
yhat.rf1 <- predict(rf1,dat_test2)
yhat.rf1
